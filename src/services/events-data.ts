...
class EventsDataService extends TransactionBaseService {
  static identifier = "eventsDataService";
  ...
  private humanPrice_(amount, currency) {
    if (!amount) {
      return "0.00";
    }
  ...
  private normalizeThumbUrl_(url) {
    if (!url) {
      return null;
    }
  ...
  private async extractLocale(fromOrder) {
  ...
  private async getOrderData(event: string, data: any) {
  ...
  private async getCartData(event: string, data: any) {
    let cartData = await this.cartService_.retrieveWithTotals(data.id);
    return cartData;
  }

  private async getPaymentData(event: string, data: any) {
    let paymentData = await this.paymentService_.retrieve(data.id);

    return paymentData;
  }

  private async getCustomerData(event: string, data: any) {
  ...
  async fetchData(event: string, data: any) {
